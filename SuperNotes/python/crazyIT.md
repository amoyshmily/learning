...
如果raise语句处在except代码块中，则会自动引发当前上下文激活的异常；否则，默认引发
RuntimeError异常。
...
# 常用方法
with_traceback():处理异常的传播轨迹。通过查看异常的轨迹，可以追踪异常触发的
源头，也可以看到异常一路触发的轨迹。
...
#### 自定义异常类
```
用户自定义的异常类都应该继承自Exception基类或者Exception的子类。

语法：
class MyException(Exception)):
    pass
    
该异常类不需要类体定义，因此使用pass占位即可。异常类的类名应准确描述异常情形。
```

#### 异常轨迹
```
只要异常没有被完全捕获，那么异常就会从发生异常的函数或者方法逐渐向外传播，首先
传给函数或方法的调用者，该函数或方法再传给其调用者，直至最后传到Python解释器。
此时，解释器会终止该程序，并打印异常的传播轨迹信息。
...
Traceback (most recent call last):
  File "C:\Users\Administrator\Desktop\2.py", line 11, in <module>
    fn_one()
  File "C:\Users\Administrator\Desktop\2.py", line 5, in fn_one
    fn_two()
  File "C:\Users\Administrator\Desktop\2.py", line 8, in fn_two
    raise MyException('引发自定义异常')
    
Python专门提供了traceback模块来处理异常传播轨迹。
（1）traceback.print_exc():将异常传播轨迹信息输出到控制台或者指定文件中。
完整形式是print_exc([limit[,file]])，它会自动处理当前except代码块所捕获的
异常。
limit:用于限制显示异常传播的层数，如果不设置，默认全部显示。例如，如果函数A调用
函数B，函数B调用函数C，而在C中出现了异常，如果limit设为1，则只显示A函数里发生的
异常。
